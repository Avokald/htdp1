...
;; produces true if los contains s
(check-expect (contains? "UBC" empty) false)
(check-expect (contains? "UBC" (cons "McGill" empty)) false)
(check-expect (contains? "UBC" (cons "UBC" empty)) true)
...
(check-expect (contains? "New-York" (cons "UBC" (cons "McGill" empty))) false)
...
;; given fn and (list n1 n2 ...) produce (list (fn n1) (fn n2) ...)
...
(check-expect (map2 string-length (list "hello" "new" "beautiful" "world")) (list 5 3 9 5))
...
;; given predicate pred and (list n0 n1 ...) produces a list of values that are evaluated by pred to true
